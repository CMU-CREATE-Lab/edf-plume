  ...
  */
  ...
  function setHelperLayerPosition(helperLayer) {
    if (helperLayer) {
      ...
      // @pdille change start
      var currentElementCustomPaddings = currentElement.highlightPaddings || {};
      // @pdille change end
      ...
      var top = elementPosition.top - widthHeightPadding / 2;
      var left = elementPosition.left - widthHeightPadding / 2;

      if (Object.keys(currentElementCustomPaddings).length !== 0) {
        width += (currentElementCustomPaddings.width || 0);
        height += (currentElementCustomPaddings.height || 0);
        top += (currentElementCustomPaddings.top || 0);
        left += (currentElementCustomPaddings.left || 0);
      }
      // @pdille change end
  ...
  function placeTooltip(targetElement, tooltipLayer, arrowLayer, hintMode) {
    ...
    if (currentTooltipPosition !== "floating" && this._options.autoPosition) {
      currentTooltipPosition = _determineAutoPosition.call(this, helperLayer, tooltipLayer, currentTooltipPosition);
    }
    ...
    // @pdille change start
    helperLayerOffset = getOffset(helperLayer);
    // @pdille change end
    ...
    switch (currentTooltipPosition) {
      case "top-right-aligned":
        ...
        checkLeft(helperLayerOffset, tooltipLayerStyleRight, tooltipOffset, tooltipLayer);
        tooltipLayer.style.bottom = "".concat(helperLayerOffset.height + 10, "px");
        break;

      case "top-middle-aligned":
        arrowLayer.className = "introjs-arrow bottom-middle";
        var tooltipLayerStyleLeftRight = helperLayerOffset.width / 2 - tooltipOffset.width / 2; // a fix for middle aligned hints

        if (hintMode) {
          tooltipLayerStyleLeftRight += 5;
        }

        if (checkLeft(helperLayerOffset, tooltipLayerStyleLeftRight, tooltipOffset, tooltipLayer)) {
          tooltipLayer.style.right = null;
          checkRight(helperLayerOffset, tooltipLayerStyleLeftRight, tooltipOffset, windowSize, tooltipLayer);
        }
      ...
      case "top":
        ...
        checkRight(helperLayerOffset, tooltipLayerStyleLeft, tooltipOffset, windowSize, tooltipLayer);
        tooltipLayer.style.bottom = "".concat(helperLayerOffset.height + 10, "px");
        break;

      case "right":
        tooltipLayer.style.left = "".concat(helperLayerOffset.width + 10, "px");

        if (helperLayerOffset.top + tooltipOffset.height > windowSize.height) {
          // In this case, right would have fallen below the bottom of the screen.
          // Modify so that the bottom of the tooltip connects with the target
          arrowLayer.className = "introjs-arrow left-bottom";
          tooltipLayer.style.top = "-".concat(tooltipOffset.height - helperLayerOffset.height - 10, "px");
        } else {
      ...
      case "left":
        ...
        tooltipLayer.style.right = "".concat(helperLayerOffset.width + 10, "px");
        break;
      ...
      case "bottom-right-aligned":
        ...
        checkLeft(helperLayerOffset, tooltipLayerStyleRight, tooltipOffset, tooltipLayer);
        tooltipLayer.style.top = "".concat(helperLayerOffset.height + 10, "px");
        break;

      case "bottom-middle-aligned":
        arrowLayer.className = "introjs-arrow top-middle";
        tooltipLayerStyleLeftRight = helperLayerOffset.width / 2 - tooltipOffset.width / 2; // a fix for middle aligned hints
  ...
  function fetchIntroSteps(targetElm) {
    ...
    if (this._options.steps) {
      ...
      forEach(this._options.steps, function (step) {
        ...
        // @pdille change start
        // TODO: Consider changing API to have user only pass in selector string and then always here do querySelector lookup
        // @pdille change end
        if (typeof currentItem.element === "string") {
          //grab the element with given selector from the page
          currentItem.element = document.querySelector(currentItem.element);
  ...
  function refresh(refreshSteps) {
    ...
    // @pdille change start
    var currentStepItem = this._introItems[this._currentStep];

    // Check if the element still exist in the DOM (either moved, and thus became stale, or is no longer in the DOM)
    // and attempt to find it again if it not.
    if (currentStepItem.selector && !document.contains(currentStepItem.element)) {
      currentStepItem.element = document.querySelector(currentStepItem.selector);
    }
    // @pdille change end
    ...
    if (refreshSteps) {
      ...
      _recreateBullets.call(this, referenceLayer, currentStepItem);
    ...
    if (this._currentStep !== undefined && this._currentStep !== null) {
      ...
      if (currentStepItem.element && oldtooltipContainer && oldArrowLayer) {
        placeTooltip.call(this, currentStepItem.element, oldtooltipContainer, oldArrowLayer);
      }
    } //re-align hints
