...
(function ($) {
    ...
    Mobileselect.prototype = {
        ...
        _buildHTML: function () {
            ...
            this.$c = $('<div id=' + this.id + ' class="mobileSelect-container"></div>').addClass(this.theme).appendTo('body');
            ...
            /*
             * set text for title and buttons.
             */
            this.$c.find('.mobileSelect-title').html(this.title).end().find('.mobileSelect-savebtn').html(this.buttonSave).end().find('.mobileSelect-selectallbtn').html(this.buttonSelectAll).end().find('.mobileSelect-clearbtn').html(this.buttonClear).end().find('.mobileSelect-cancelbtn').html(this.buttonCancel).end();

            this.$listcontainer = this.$c.find('.list-container');

            if (!this.filterable) {
                this.$c.find('.mobileSelect-filter').remove();
            } else if (this.filterPlaceholder) {
                this.$c.find('.mobileSelect-filter').attr("placeholder", this.filterPlaceholder);
            }

            if (!this.isMultiple) {
                this.$c.find('.mobileSelect-clearbtn').remove();
                this.$c.find('.mobileSelect-selectallbtn').remove();
            } else {
        ...
        _bindEvents: function () {
            ...
            this.$c.find('.mobileSelect-filter').on('input', function (e) {
                that.$listcontainer.find($('.no-result')).remove();
                ...
                if (string === "") {
                    ...
                    if (that.$listcontainer.find(':visible').length == 0) {
                        that.$listcontainer.append("<div class='no-result'>No result found.</div>");
                    }
                }
            });
        ...
        show: function () {
            ...
            var $filterField = this.$c.find('.mobileSelect-filter');
            if (this.filterable && $filterField.val()) {
              $filterField.val('').trigger("input");
            }
        ...
        _setUserOptions: function () {
            ...
            if (this.$e.data('btntext-select-all') !== undefined) {
                this.buttonSelectAll = this.$e.data('btntext-select-all');
            }
