...
(function() {
   ...
   org.bodytrack.grapher.DateAxis = function(elementId) {
      ...
      /**
       * Enable or disable drawing the cursor line.  The cursor is not drawn if <code>enableState</code>
       * is false.
       *
       * @param {boolean} enableState - whether to enable or disable showing the cursor line.
       */
       this.setCursorEnabled = function(enableState) {
         wrappedAxis.setCursorEnabled(enableState);
      };
      ...
      this.setTimeZone = function(zoneName) {
        if (typeof wrappedAxis.setTimeZone === 'function') {
          wrappedAxis.setTimeZone(zoneName);
        }
        else {
          console.log("WARN: the underlying grapher does not support setting time zone.");
        }
      };

      this.setCursorDraggable = function(doDrag) {
        if (typeof wrappedAxis.setCursorDraggable === 'function') {
          wrappedAxis.setCursorDraggable(doDrag);
        }
        else {
          console.log("WARN: the underlying grapher does not support setting cursor draggable state.");
        }
      };
   ...
   org.bodytrack.grapher.PlotManager = function(dateAxisElementId, minTimeSecs, maxTimeSecs) {
      ...
      this.setTimeZone = function(zoneName) {
        if (!zoneName) return;
        dateAxis.setTimeZone(zoneName);
      };

      this.setCursorDraggable = function(doDrag) {
        if (typeof(doDrag) !== "boolean") return;
        dateAxis.setCursorDraggable(doDrag);
      };
